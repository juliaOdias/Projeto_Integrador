...
def excluir_pet(pet_id):
    try:
        ...
        cursor.close()
        conn.close()
@app.route('/filtrar_pets_por_cor_dos_olhos', methods=['POST'])
def filtrar_pets_por_cor_dos_olhos():
    try:
        # Consulta o banco de dados para obter os pets com as cores dos olhos selecionadas
        conn = mysql.connector.connect(
            host='localhost',
            port=3306,
            user='root',
        ...
        cursor = conn.cursor()
        selected_filters = request.json
        selected_colors = request.form.getlist('color')
        selected_idade = request.form.getlist('idade')
        selected_especie = request.form.getlist('especie')
        selected_genero = request.form.getlist('genero')
        selected_porte = request.form.getlist('porte')
        selected_castrado = request.form.getlist('castrado')
        selected_cor = request.form.getlist('cor')
        selected_situacao = request.form.getlist('situacao')

        query = """
            SELECT Nome, Raca, Especie, Genero, Porte, Idade, Cor, Castrado, Doenca, Medicamento, Agressivo, CorDosOlhos, Situacao, imagem
            FROM cadastropet
            WHERE 1
        """

        ## verifica se existe alguma coisa dentro dessa tupla
        if len(selected_especie) > 0:
            query += "AND especie = "  + selected_especie[0]


        print(query)

        # Merge all the selected values
        all_selected_values = selected_colors + selected_especie + selected_genero + selected_porte + selected_idade + selected_cor + selected_castrado + selected_cor + selected_situacao

        # Execute the query with the merged values
        cursor.execute(query, all_selected_values)
        data = cursor.fetchall()

        # Criar um array de pets
        pets = []
        for row in data:
            pet = {
                'nome': row[0],
